...
const PaymentHistory = () => {
  ...
  const { data: payments = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosSecure("/member-payments");
      return res.data;
    },
  });
  ...
  return (
    <div className="bg-base-200 p-4">
      ...
      {isLoading ? (
            ...
            <tbody>
              {payments.map((payment) => (
                <tr key={payment._id}>
                  ...
                  <td>
                    <div className="badge border-2">{payment.paymentType}</div>
                  </td>
        ...
        <p className="text-center text-2xl mt-10">No transactoins yet!</p>
      )}
    </div>
  );
};
